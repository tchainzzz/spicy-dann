...
def train_step(iteration, model, train_loader, grouper, loss_class, loss_domain, optimizer, limit_batches=-1):
    ...
    for i, (x, y_true, metadata) in tqdm(enumerate(train_loader)):
        ...
        all_domain_true = raw_metadata.topk(torch.unique(raw_metadata).numel()).indices # all domain labels are unique and can be deterministically ordered, so use topk

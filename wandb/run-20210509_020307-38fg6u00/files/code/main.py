...
def train(train_loader, val_loader, model, grouper, n_epochs, get_train_metrics=True, save_every=5, max_val_batches=100, binary=False):
    ...
    for i in range(n_epochs):
        ...
        val_metrics = evaluate(i, model, val_loader, limit_batches=max_val_batches, binary=binary)
        log('val', val_metrics)
...
def compute_metrics(all_class_true, all_class_pred, all_domain_true, all_domain_pred, binary=False):
    ...
    metrics = {
        ...
        "class/precision": class_prec,
        "class/recall":class_rec,
        "class/f1": class_f1,
        "domain/precision": domain_prec,
        "domain/recall": domain_rec,
        "domain/f1": domain_f1
    }
    return metrics
...
def evaluate(iteration, model, val_loader, grouper, limit_batches=-1):
    ...
    model.eval()
    with torch.no_grad():
        for i, (x, y_true, metadata) in enumerate(tqdm(val_loader)):
            if i == limit_batches:
                logger.warning(f"limit_batches set to {limit_batches}; early exit")
                break
            raw_metadata = grouper.metadata_to_group(metadata)
